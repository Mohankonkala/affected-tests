...
class AffectedTestsPluginTest {
    ...
    @Test
    void kotlinEnabledPropertyAcceptsHumanFriendlyAliases() {
        // Mirrors {@link #parallelDiscoveryPropertyAcceptsHumanFriendlyAliases}
        // for the Phase 2 PR #3 system property
        // {@code -Daffected-tests.kotlin.enabled=...}. The accept-set
        // is intentionally identical to the parallelDiscovery alias
        // set so adopters can flip both knobs with the same
        // muscle memory.
        org.gradle.api.logging.Logger log = org.gradle.api.logging.Logging
                .getLogger(AffectedTestsPluginTest.class);

        assertTrue(AffectedTestsPlugin.parseKotlinEnabledProperty("true", log));
        assertTrue(AffectedTestsPlugin.parseKotlinEnabledProperty("True", log));
        assertTrue(AffectedTestsPlugin.parseKotlinEnabledProperty("TRUE", log));
        assertTrue(AffectedTestsPlugin.parseKotlinEnabledProperty("1", log));
        assertTrue(AffectedTestsPlugin.parseKotlinEnabledProperty("on", log));
        assertTrue(AffectedTestsPlugin.parseKotlinEnabledProperty("ON", log));
        assertTrue(AffectedTestsPlugin.parseKotlinEnabledProperty("yes", log));

        assertFalse(AffectedTestsPlugin.parseKotlinEnabledProperty("false", log));
        assertFalse(AffectedTestsPlugin.parseKotlinEnabledProperty("False", log));
        assertFalse(AffectedTestsPlugin.parseKotlinEnabledProperty("0", log));
        assertFalse(AffectedTestsPlugin.parseKotlinEnabledProperty("off", log));
        assertFalse(AffectedTestsPlugin.parseKotlinEnabledProperty("no", log));
    }

    @Test
    void kotlinEnabledPropertyRejectsTyposToTheSafeDefault() {
        // Polarity flip vs parallelDiscovery — the safe default
        // during the Phase 2 rollout window is Kotlin OFF, so a typo
        // must NOT silently flip Kotlin ON. An adopter who typo'd
        // {@code -Daffected-tests.kotlin.enabled=tru} expects a no-op,
        // not a half-bootstrapped Kotlin language parser. Once PR #4
        // flips the canonical default to ON the safe-default branch
        // here will need to be updated deliberately.
        org.gradle.api.logging.Logger log = org.gradle.api.logging.Logging
                .getLogger(AffectedTestsPluginTest.class);

        assertFalse(AffectedTestsPlugin.parseKotlinEnabledProperty("tru", log),
                "Typo'd 'tru' must NOT silently enable Kotlin AST — "
                        + "fall back to the safe default (Kotlin OFF) "
                        + "during the Phase 2 rollout window");
        assertFalse(AffectedTestsPlugin.parseKotlinEnabledProperty("flase", log),
                "Typo'd 'flase' must NOT silently enable Kotlin AST");
        assertFalse(AffectedTestsPlugin.parseKotlinEnabledProperty("enable", log),
                "Common-sense-but-unsupported alias must NOT silently enable — "
                        + "if we expand the alias set later this test will "
                        + "trip and we'll know to update it deliberately");
        assertFalse(AffectedTestsPlugin.parseKotlinEnabledProperty("activate", log),
                "Random invalid value must fall back to the safe default");
    }
}

...
- **Kotlin: AST parser shipped, default off (PR #3 of issue [#76](https://github.com/vedanthvdev/affected-tests/issues/76)).** `.kt` sources have participated in the path-derived mapping pipeline since PR #1: a `Foo.kt` with a sibling `FooTest.kt` selects via the naming-convention strategy, top-level functions in `Util.kt` emit a synthetic `UtilKt` FQN, and `--explain` surfaces a `Kotlin source mapped via filename only` hint. As of PR #3 of the rollout, full Kotlin AST parity lands behind a system-property gate: setting `-Daffected-tests.kotlin.enabled=true` routes `.kt` files through `kotlin-compiler-embeddable`'s PSI walk, which feeds the AST-driven strategies (UsageStrategy direct-import, ImplementationStrategy supertype walk, TransitiveStrategy reference graph) the same `FileMetadata` shape Java produces. The flag stays **off by default** for the rollout window so existing adopters see zero change. PR #4 will flip the default. Three Phase-2 limitations carry into the next minor: (1) `@file:JvmName(...)` is detected and surfaced via `--explain` but the synthetic class name still uses the `<basename>Kt` shape (a follow-up issue tracks honouring the override); (2) the parser focuses on Kotlin/JVM and prunes the JS / Native / WASM / FIR-analysis / IR-codegen subsystems from the shadow JAR for size, so any future Kotlin Multiplatform feature that depends on those subsystems would not be reachable through this plugin; (3) the `KotlinCoreEnvironment` underpinning the parser registers a process-wide static MockApplication on first use — adopters who flip `-Daffected-tests.kotlin.enabled=true` on a multi-module Gradle build with `org.gradle.parallel=true` should expect bootstrap collisions across subprojects (the rollout flag is off by default precisely so this only matters for adopters opting in, and PR #4's flag-flip carries the multi-module-parallel acceptance gate). See [`docs/PHASE-2-KOTLIN-AST.md`](docs/PHASE-2-KOTLIN-AST.md) for the four-PR sequence.
